...
export class Diagram {
 constructor(canvas) {
  ...
  this.connectable = true;       // false in read-only viewer — disables manual-link creation
 ...
 _render() {
  ...
  if (this.connectable && this.hoverConn && !this.linking && !this.drag && !this.dragGroup && !this.pan) {
 ...
 _connectorAt(sx, sy) {
  if (!this.connectable) return null;
  const w = this.screenToWorld(sx, sy);
